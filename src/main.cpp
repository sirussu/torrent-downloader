...
int main(int argc, char const *argv[]) {
    ...
    if (command == "torrent") {
        if (argc < 4) {
            OutStatus(Trinity::StringFormat(UpdateJson, "Please provide torrent file and output dir", true,
                                            "system-error"));
            return 1;
        }
            ...
            for (;;) {
                ...
                for (lt::alert const *a : alerts) {
                    if (auto at = lt::alert_cast<lt::add_torrent_alert>(a)) {
                        ...
                        OutStatus(
                                Trinity::StringFormat(UpdateJson, h.torrent_file()->name(), false, "download-started"));
                    }
                    ...
                    if (auto st = lt::alert_cast<lt::state_update_alert>(a)) {
                        ...
                        OutStatus(Trinity::StringFormat(StatusJson, updateAt, s.total, s.total_done, s.progress,
                                                        state(s.state), s.download_payload_rate));
                    }
                }
